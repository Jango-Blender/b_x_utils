...
pub const object_types = opaque {
    ...
    pub const UnicodeObject = extern struct {
        ...
        pub fn toObject(self: object_types.UnicodeObject) object_types.Object {
            return .{ .ptr = @ptrCast(@alignCast(self.ptr)) };
        }
        ...
        pub fn from(buffer: []const u8, encoding: Encoding) GenericError!object_types.UnicodeObject {
            // const codec = @tagName(encoding);
            _ = encoding;
            const codec: [*:0]const u8 = "cp1252";
            const ptr = py.PyUnicode_Decode(buffer.ptr, @intCast(buffer.len), codec, null) orelse {
                std.debug.print("Failed to convert ({s}) to py using {s}\n", .{ buffer, codec });
                py.PyErr_Print();
                return error.Failed;
            };
            return .{ .ptr = @ptrCast(ptr) };
