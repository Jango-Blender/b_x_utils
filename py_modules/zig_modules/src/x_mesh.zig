...
//             },
//             else => {
//                 std.debug.print("found an unknown vertex map type 0x{x} @ 0x{x}\n", .{ vmflag, fixed.r.seek });
...
// const PhysicsSphere = extern struct {
//     pos: Vector3df = .{},
//     size: f32 = 0.0,
// };
...
//         .v3 = if (constraint_type & 4 != 0) try fixed.take(Vector3df) else .{},
//     };
// }
...
//                         constraint.* = try read_constraint(fixed);
//                     }
//                     try chunks.set(.motion_constraints, motion_constraints);
//                 },
//                 0xDDB3 => { //Sound
//                     try chunks.set(.sound, try fixed.take(PhysicsSound));
...
//             .raw_data = raw_data,
//             .density = density,
//             .spheres = chunks.get(.spheres) catch &.{},
//             .motion_constraints = chunks.get(.motion_constraints) catch &.{},
//             .sound = chunks.get(.sound) catch .{},
//         };
//     } else {
...
//             try fixed.readSlice(Vector3df, spheres);
...
//             //Keep only the first entry I guess.
//             0x00003D00 => chunks.set(.facechunk, try read_faces(arena, &fixed, meshmaps, chunklength, is_prop)) catch {},
//             0x3DD0B000 => chunks.set(.physics, try read_physics(arena, &fixed, chunklength, is_prop)) catch {},
//             0x3D0CEC00 => chunks.set(.statics, try read_statics(arena, &fixed, chunktag, chunklength, is_prop)) catch {},
//             0x3DD0C000 => { //Softbody
...
//         .vertexbundle = vertexbundle,
//         .meshmaps = meshmaps,
//         .facechunk = chunks.get(.facechunk) catch .{},
//         .physics = chunks.get(.physics) catch .{},
//         .statics = chunks.get(.statics) catch .{},
//         .softbody = chunks.get(.softbody) catch .{},
//     };
// }
...
//     const py_verts = try py_wr.vector3df_slice_to_python(mr.vertexbundle.verts);
//     const material_names: pyo.TupleObject = try .initPlaceholders(mr.meshmaps.materials.len);
//     for (mr.meshmaps.materials, 0..) |mat, i| {
//         const clean_name = std.mem.sliceTo(&mat.name.bytes, 0);
//         material_names.setUnchecked(i, try pyo.UnicodeObject.from(clean_name, .cp1252));
//     }
//     const py_fc = try facechunk_to_py(mr.vertexbundle, mr.meshmaps, mr.facechunk, is_prop);

//     result.setUnchecked(0, py_verts);
//     result.setUnchecked(1, py_fc.face_vert_indices);
//     result.setUnchecked(2, py_fc.loop_uvs);
//     result.setUnchecked(3, py_fc.material_indices);
//     result.setUnchecked(4, material_names);
//     result.setUnchecked(5, py_fc.faceints);
//     result.setUnchecked(6, py_fc.faceflags);
//     return result;
// }
...
//         py.PyErr_SetString(py.PyExc_RuntimeError, "parse_mesh failed");
//         return null;
//     };

//     const ret_object = meshresult_to_py(mesh_result, is_prop) catch {
//         py.PyErr_SetString(py.PyExc_RuntimeError, "meshresult_to_py failed");
//         return null;
//     };
